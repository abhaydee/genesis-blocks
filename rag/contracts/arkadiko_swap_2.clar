...
(define-read-only (is-registered-swap-token (token principal))
  (is-some (map-get? registered-swap-tokens { swap-token: token }))
)

(define-private (register-swap-token (token principal))
  (begin
    (asserts! (not (is-registered-swap-token token)) (err ERR-WRONG-SWAP-TOKEN))
    (ok (map-set registered-swap-tokens { swap-token: token } { registered: true }))
  )
)
